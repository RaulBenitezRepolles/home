...
class calendario(HydraHeadApp):
	...
	def run(self):
		try:
			...
			#################### init ####################
			variables = table_variables('calendario','Evento TEXT,Comentarios TEXT,Fecha DATETIME')
			#table_drop(table)
			table_create('calendario')
			
			c1, c2= st.columns((1,4))
			tipo_productos=c1.selectbox('Tipo de Evento', ['Activos','Inactivos','Todos'])
			if tipo_productos=='Activos':
				df_calendario=table_view('calendario')
				with st.form(key='Desactivar_id_activos'):
					selection = show_aggrid(df_calendario)
					submit_button = st.form_submit_button(label='Desactivar')
					if submit_button:
						selection = [i for i in selection]					
						for id in selection:
							row_deactivate('calendario',id['id'],variables)
						st.experimental_rerun()
				c1, c2, c3, c4 = st.columns((1,1,1,1))
				Evento = c1.text_input("Añadir Evento")#c1.text_input("Seleccionar Artículo")
				Comentarios = c2.text_input("Comentarios")
				date = c3.date_input('Fecha')
				hour = c4.time_input('Hora',value=now_time)
				Fecha = str(date) +' '+ str(hour)
				values='"'+'","'.join(str(i) for i in [Evento,Comentarios,Fecha])+'"'
				if st.button("Añadir",key='añadir evento'):
					row_add('calendario',values)
					st.experimental_rerun()
					
			if tipo_productos=='Inactivos':
				df_calendario=table_view('calendario','0')
				with st.form(key='Activar_id_inactivos'):
					...
					if submit_button:
						...
						for id in selection:
							...
							row_activate_hist('calendario',id['Traza'])
							row_activate('calendario',fields_db_name,values_add)
						st.experimental_rerun()
			
			if tipo_productos=='Todos':
				df_calendario=table_view('calendario','None','fields_id_db')
				selection = show_aggrid(df_calendario,checkbox=False)
